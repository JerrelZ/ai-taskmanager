...
@endphp
<div
    wire:key="ticket-card-{{ $task->id }}"
    wire:sort:item="{{ $task->id }}"
    wire:click="openTask({{ $task->id }})"
    @class([
        'group cursor-pointer rounded-lg border border-s-4 border-zinc-200 bg-white p-3 shadow-sm transition hover:border-zinc-300 dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-zinc-600',
        'border-s-'.$task->project->color.'-500',
        'ring-1 ring-brand-400' => in_array($task->id, $selectedTickets),
...
>
    ...
    <div class="mb-1.5 flex items-center justify-between gap-2">
        <span class="flex min-w-0 items-center gap-1.5">
            @if ($isTeam)
                <span wire:sort:ignore class="shrink-0">
        ...
        <div class="flex shrink-0 items-center gap-1">
            ...
            {{-- Mobile: tap to move the card to another column. --}}
            <flux:dropdown wire:sort:ignore position="bottom" align="end" class="lg:hidden">
                ...
                <flux:menu>
                    @foreach (\App\Enums\TaskStatus::cases() as $columnStatus)
                        <flux:menu.item x-on:click.stop wire:click="setStatus({{ $task->id }}, '{{ $columnStatus->value }}')"
        ...
        <div class="flex shrink-0 items-center gap-0.5">
            @if ($stale)
                <flux:tooltip :content="__('Niet bijgewerkt sinds').' '.$touched?->translatedFormat('j M Y')">
                    <flux:button wire:click.stop="markReviewed({{ $task->id }})" variant="subtle" size="xs" icon="exclamation-triangle" inset="top bottom"
