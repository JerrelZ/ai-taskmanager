...
<div wire:key="ticket-{{ $task->id }}" wire:sort:item="{{ $task->id }}" @class([
    ...
    <div wire:sort:handle
        class="hidden cursor-grab text-zinc-300 opacity-0 transition group-hover:opacity-100 sm:block dark:text-zinc-600"
        title="{{ __('Sleep om prioriteit te bepalen') }}">
    ...
    {{-- Title + project --}}
    <button type="button" wire:click="openTask({{ $task->id }})"
        ...
        <span class="flex items-center gap-1.5 text-xs text-zinc-400">
            ...
            <div class="-mt-1">@include('livewire.partials.linear-badge', ['task' => $task])</div>
    ...
    @if ($stale)
        <flux:tooltip
            :content="__('Niet bijgewerkt sinds').' '.$touched?->translatedFormat('j M Y').' ('.$touched?->diffForHumans(short: true).')'">
    ...
    <div class="hidden items-center gap-3 text-xs text-zinc-400 sm:flex">
        @if ($progress['total'] > 0)
            <span class="flex items-center gap-1">
                <flux:icon name="list-bullet" variant="micro" />{{ $progress['done'] }}/{{ $progress['total'] }}
            </span>
        @endif
        @if ($task->comments_count > 0)
            <span class="flex items-center gap-1">
                <flux:icon name="chat-bubble-oval-left" variant="micro" />{{ $task->comments_count }}
            </span>
        @endif
    </div>
    ...
    <div wire:sort:ignore
        class="hidden shrink-0 items-center gap-0.5 opacity-0 transition group-hover:opacity-100 sm:flex">
        @if ($stale)
            <flux:tooltip :content="__('Markeer als bijgewerkt')">
                <flux:button wire:click="markReviewed({{ $task->id }})" variant="subtle" size="xs"
                    icon="check" inset="top bottom" />
            </flux:tooltip>
        @endif
        @if (auth()->user()?->canCopyPrompt())
            <flux:tooltip :content="__('Kopieer als AI-prompt')">
                <flux:button wire:click="copyPrompt({{ $task->id }})" variant="subtle" size="xs"
