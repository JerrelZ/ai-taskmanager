...
<body class="min-h-screen bg-zinc-50 font-sans text-zinc-800 antialiased dark:bg-zinc-950 dark:text-zinc-200">
    <flux:sidebar sticky collapsible="mobile"
        class="border-e border-zinc-200 bg-zinc-100/50 max-lg:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900/40 max-lg:dark:bg-zinc-900">
        <flux:sidebar.header>
            <a href="{{ route('tickets.index') }}" wire:navigate class="flex items-center gap-2 px-1 py-1">
                <span
                    class="flex size-7 items-center justify-center rounded-full bg-brand-600 font-display text-lg leading-none text-white">t</span>
        ...
        <flux:sidebar.nav>
            <flux:sidebar.item icon="queue-list" :href="route('tickets.index')"
            ...
            <flux:sidebar.item icon="chat-bubble-left-right" :href="route('messages.index')"
                :current="request()->routeIs('messages.*')" :badge="$unreadMessages > 0 ? $unreadMessages : null"
            ...
            <flux:sidebar.item icon="rectangle-stack" :href="route('projects.index')"
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Projecten')" expandable :expanded="true" class="grid">
                    @foreach ($sidebarProjects as $sidebarProject)
                        <flux:sidebar.item :href="route('projects.board', $sidebarProject)"
                            :current="request()->routeIs('projects.board') && request()->route('project')?->is($sidebarProject)"
                            wire:navigate>
        ...
        @if (auth()->user()->isAdmin())
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Beheer')" class="grid">
                    ...
                    <flux:sidebar.item icon="users" :href="route('team.index')"
    ...
    <flux:header class="lg:hidden">
        ...
        <flux:dropdown position="top" align="end">
            <flux:profile :initials="auth()->user()->initials()" :avatar="auth()->user()->avatar_url" icon-trailing="chevron-down" />

            <flux:menu>
                <flux:menu.radio.group>
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                            <flux:avatar :name="auth()->user()->name" :initials="auth()->user()->initials()" :src="auth()->user()->avatar_url" />
